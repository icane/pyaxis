...
class TestPyaxis(unittest.TestCase):
    ...
    def test_uri_type(self):
        """uri_type() should be capable of discriminating between file and URL."""

        self.assertEqual(pyaxis.uri_type('2184.px'), 'FILE', 'Uri type differs!')
        self.assertEqual(pyaxis.uri_type('http://www.ine.es/jaxiT3/files/t/es/px/2184.px'),
    ...
    def test_metadata_extract_split(self):
        ...
        self.assertEqual(metadata_elements[13], 'STUB="Comunidades y Ciudades Autónomas",'
                         '"Grupos ECOICOP"', 'Metadata differs!')
        self.assertEqual(metadata_elements[14], 'HEADING="Tipo de dato",'
                         '"Periodo"', 'Metadata differs!')
        self.assertEqual(metadata_elements[22], 'UNITS="  Índice,   Tasas"', 'Metadata differs!')
        metadata = pyaxis.metadata_split_to_dict(metadata_elements)
        self.assertEqual(metadata['VALUES(Comunidades y Ciudades Autónomas)'][2], 'Aragón',
                         'Metadata differs!')
        self.assertEqual(metadata['VALUES(Comunidades y Ciudades Autónomas)'][4], 'Balears, Illes',
                         'Metadata differs!')

    def test_get_dimensions(self):
        """get_dimensions() should return two lists with dimension names and members. """
    ...
    def test_parse(self):
        """ parse() should parse a pc-axis into a dataframe and a metadata dict."""

        parsed_pcaxis = pyaxis.parse(self.base_path + '2184.px', encoding='ISO-8859-2')
        self.assertEqual(parsed_pcaxis['DATA'].dtypes['DATA'], 'float64')
        self.assertEqual(parsed_pcaxis['METADATA']['VALUES(Índices y tasas)'].index('Índice'),
                         0, 'Dictionary index differs!')
        self.assertGreater(len(parsed_pcaxis['DATA']), 9500)

    def test_to_csv(self):
        """ Data returned by parse() should be able to be converted to CSV format."""

        # fichero 1
        parsed_pcaxis = pyaxis.parse('http://www.ine.es/jaxiT3/files/es/2184.px',
        ...
        # fichero 2
        parsed_pcaxis = pyaxis.parse('http://www.ine.es/jaxiT3/files/es/1443.px',
                                     encoding='windows-1252')
        parsed_pcaxis['DATA'].to_csv(
    ...
    def test_http_error(self):
        """ Using parse() with a non-existent URL should return a 404 exception."""
